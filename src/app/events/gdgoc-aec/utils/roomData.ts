...
//     // Try finding by ID field (case insensitive)
//     const rooms = Object.values(competitionData.rooms);
//     room = rooms.find(r => (r as any).id.toLowerCase() === roomId.toLowerCase()) || null;
//   }
...
// export const getRoomsByDifficulty = (difficulty: 'Easy' | 'Medium' | 'Hard'): Room[] => {
//   return Object.values(competitionData.rooms)
//     .filter((room: any): room is Room => isValidRoom(room) && room.difficulty === difficulty);
// };
...
export interface Room {
  ...
  testInput: string;
  expectedOutput: string;
  testCasesCount?: number; // Make this optional since it's in your JSON but may not be needed
}
...
// Helper function to safely convert any room data to Room type
const convertToRoom = (room: any): Room | null => {
  ...
  return {
    ...
    testCasesCount: room.testCasesCount
  };
};
...
export const getRoomsByDifficulty = (difficulty: 'Easy' | 'Medium' | 'Hard'): Room[] => {
  return Object.values(competitionData.rooms)
    .map(convertToRoom)
    .filter((room): room is Room => room !== null && room.difficulty === difficulty);
};