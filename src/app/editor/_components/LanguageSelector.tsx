...
function LanguageSelector({ fullWidth = false }: LanguageSelectorProps) {
  ...
  return (
    <div className={`relative ${fullWidth ? 'w-full' : ''}`} ref={dropdownRef}>
      <motion.button
        ...
        className={`${fullWidth ? 'w-full' : ''} group relative flex items-center gap-3 px-4 py-2.5 bg-[#1e1e2e]/80 rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700`}
      >
        {/* Decoration */}
        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-purple-500/5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
        ...
        <span className="text-gray-200 flex-1 text-left group-hover:text-white transition-colors truncate">
          {currentLanguageObj.label}
        </span>

        <ChevronDownIcon 
          className={`size-4 text-gray-400 transition-all duration-300 group-hover:text-gray-300 flex-shrink-0 ${isOpen ? "rotate-180" : ""}`}
        />
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className={`absolute top-full ${fullWidth ? 'right-0' : 'left-0'} mt-2 w-full min-w-[200px] max-w-[250px] bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50`}
          >
              ...
              {Object.values(LANGUAGE_CONFIG).map((lang, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className={`relative size-8 rounded-lg p-1.5 group-hover:scale-110 transition-transform flex-shrink-0 
